...
Expect `maxSupply = 4444`, `teamReserve = 200`. Canonical SeaDrop should be **false** if you deployed with `SEADROP_ADDRESS=0x0`. After `setMetadataURIs(hidden.json, "", "")` and before `reveal()`, `tokenURI(1)` is `https://terminalpets.xyz/metadata/hidden.json` for every token (`https://terminal-pets.vercel.app/metadata/hidden.json` still resolves until DNS cutover). The recorded 46630 7-day stack used hub `/metadata/lit/1.json` after Ignite; that JSON tree is no longer hosted.

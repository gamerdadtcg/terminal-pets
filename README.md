...
Until `reveal()`, `tokenURI` uses `hiddenSvg`. After reveal, categorical space is about **826 billion** combos (`8×12×8×6×8×12×12×4×6×3×6×3×5×6×4`: shell × shell color × buttons × antenna × wallpaper × species × body × belly × eyes × pupil × mouth × cheeks × ears × accessory × generation). Accessories are None, Bow, Cap, Star, Glasses, Halo. Species use distinct fill-forward silhouettes (blob, cat, dino, fox, ghost, bunny, bird, frog, bear, robot, owl, bug) in the Sep 11 **Awaken Pet V2** chrome: dark screen bezel, speaker dots, screen gloss, split `PET // {id}` / generation footer. Dino is a round front-facing pet with ridge, snout, and a soft tail. Fox is a point-up pentagon. Bird uses a front-facing colored beak. Scarf and Pack stay removed. Each species has its own **face anchor** so eyes and accessories sit on that head. Shells have 8 outlines (egg, round, square, wave, slim, wide, octagon, clam). Art-pass stills: `artifacts/art-pass/` and `web/public/art-pass/`.

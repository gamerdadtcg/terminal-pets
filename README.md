...
Until `reveal()`, `tokenURI` uses `hiddenSvg`. After reveal, categorical space is about **826 billion** combos (`8×12×8×6×8×12×12×4×6×3×6×3×5×6×4`: shell × shell color × buttons × antenna × wallpaper × species × body × belly × eyes × pupil × mouth × cheeks × ears × accessory × generation). Accessories are None, Bow, Cap, Star, Glasses, Halo. Species use distinct fill-forward silhouettes (blob, cat, dino, fox, ghost, bunny, bird, frog, bear, robot, owl, bug). Dino Lit is a left-facing cartoon (round head, snout, four legs, tapering tail, ridge spikes). Fox is a pentagon. Bird glasses are thin rims with a short arch. Each species has its own **face anchor**. Shells are fill-only (egg, round, square, wave, slim, wide, octagon, clam) with a dark screen bezel. The device prints split `PET#{id}` / generation on the bezel. Art-pass stills: `artifacts/art-pass/` and `web/public/art-pass/`. The trait catalog at `/art-pass/traits` is the visual source of truth.

...
/// FaceAnchor + stroke-on-bezel chrome match the faces3 / good-art-ref archives:
/// fill-forward pets, fill-only shells, dark screen bezel, speaker dashes,
/// split `PET#{id}` / generation footer. Not the 3a194a4 stroked-shell dump.
library TerminalRenderer {
    ...
    function _chrome(uint256 tokenId, Roll memory a, bool mystery) private pure returns (string memory) {
        ...
        return string.concat(
            ...
            '<rect data-spk="1" x="156" y="261" width="10" height="3" rx="1" fill="#2a2430"/>',
            '<rect data-spk="1" x="170" y="261" width="10" height="3" rx="1" fill="#2a2430"/>',
            '<rect data-spk="1" x="184" y="261" width="10" height="3" rx="1" fill="#2a2430"/>',
    ...
    /// @dev Left-facing fill-forward dino from the faces3 / sample-100 archive.
    function _dinoParts(string memory body) private pure returns (string memory) {
        string memory st = string.concat(' fill="', body, '"');
        return string.concat(
            '<g data-dino="1">',
            '<ellipse cx="176" cy="206" rx="6" ry="11"',
            st,
            "/>",
            '<ellipse cx="190" cy="208" rx="6" ry="11"',
            st,
            "/>",
            '<ellipse cx="206" cy="208" rx="6" ry="11"',
            st,
            "/>",
            '<ellipse cx="220" cy="206" rx="6" ry="11"',
            st,
            "/>",
            '<ellipse data-tail="1" cx="244" cy="186" rx="20" ry="11"',
            st,
            "/>",
            '<ellipse cx="262" cy="190" rx="10" ry="7"',
            st,
            "/>",
            '<ellipse data-dino-body="1" cx="200" cy="176" rx="40" ry="28"',
            st,
            "/>",
            '<polygon data-ridge="1" points="176,158 184,124 196,156"',
            st,
            "/>",
            '<polygon points="192,152 202,118 214,152"',
            st,
            "/>",
            '<polygon points="210,154 220,122 230,156"',
            st,
            "/>",
            '<polygon points="226,158 234,132 242,164"',
            st,
            "/>",
            '<ellipse data-dino-head="1" cx="158" cy="152" rx="24" ry="22"',
            st,
            "/>",
            '<ellipse data-snout="1" cx="138" cy="160" rx="12" ry="9"',
            st,
            "/>",
            "</g>"
        );
    ...
    function _oneEye(Roll memory a, uint256 x, uint256 y, bool dino) private pure returns (string memory) {
        ...
        return string.concat(_eyeShape(a.eyes, x, y, extra), _pupil(a, x, y));
    }
    ...
    function _eyeShape(uint8 kind, uint256 x, uint256 y, string memory extra) private pure returns (string memory) {
        ...
        if (kind == 0) {
            return string.concat("<circle", at, ' r="5" fill="#fff" stroke="#1e1930" stroke-width="2"/>');
        }
        if (kind == 1) {
            return string.concat("<ellipse", at, ' rx="8" ry="5" fill="#fff" stroke="#1e1930" stroke-width="2"/>');
        }
        if (kind == 2) {
            return string.concat("<ellipse", at, ' rx="11" ry="7" fill="#fff" stroke="#1e1930" stroke-width="2"/>');
        }
        if (kind == 3) {
            return string.concat(
                "<circle",
                at,
                ' r="7" fill="#fff" stroke="#1e1930" stroke-width="2"/>',
                '<path d="M',
                (x + 6).toString(),
                " ",
                (y - 8).toString(),
        ...
        if (kind == 4) {
            return string.concat("<ellipse", at, ' rx="5" ry="11" fill="#fff" stroke="#1e1930" stroke-width="2"/>');
        }
        return string.concat(
            "<circle",
            at,
            ...
            '" cy="',
            y.toString(),
            '" r="5" fill="#fff" stroke="#1e1930" stroke-width="2"/>'
        );
    }
    ...
    function _beak(uint8 kind) private pure returns (string memory) {
        return string.concat('<polygon data-beak="1" points="218,146 246,154 218,166" fill="', _beakHex(kind), '"/>');
    }

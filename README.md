...
Until `reveal()`, `tokenURI` uses `hiddenSvg`. After reveal, categorical space is about **826 billion** combos (`8×12×8×6×8×12×12×4×6×3×6×3×5×6×4`: shell × shell color × buttons × antenna × wallpaper × species × body × belly × eyes × pupil × mouth × cheeks × ears × accessory × generation). Accessories are None, Bow, Cap, Star, Glasses, Halo. Species use distinct silhouettes (blob, cat, dino, fox, ghost, bunny, bird, frog, bear, robot, owl, bug). Dino / Ghost / Bird / Robot are drawn to read as those names. Dino Lit is a left-facing blob (round head + snout, four spikes, simple legs, tail). Bird glasses are thin rims on the pupils with a short arch — not a second pair of eyes. Each species has its own **face anchor** (eye center, gap, mouth, hat) so eyes and accessories sit on that head. Shells are fill-only (egg, round, square, wave, slim, wide, octagon, clam) with a dark screen bezel; pets keep dark `#2a2430` outlines. The device prints split `PET#{id}` (left) and generation (right) on the bezel. Art-pass stills: `artifacts/art-pass/` and `web/public/art-pass/`.
